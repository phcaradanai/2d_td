...
# ── Status colors (distinct, neon-tinted to match sci-fi theme) ──────────────
const C_SLOW   := Color(0.50, 0.95, 1.00, 1.0)   # cyan-white  — slow / freeze
const C_ROOT   := Color(0.15, 0.90, 0.45, 1.0)   # teal-green  — root / snare
const C_BURN   := Color(1.00, 0.32, 0.05, 1.0)   # orange-red  — fire DoT
const C_POISON := Color(0.52, 0.12, 0.88, 1.0)   # purple      — nature/dark DoT
const C_VULN   := Color(0.80, 0.08, 0.88, 1.0)   # magenta     — vulnerability
...
func _draw_body_ring(st: Dictionary) -> void:
	var r := RING_R
	# Root: solid teal ring — strongest visual
	if st["root"]:
		draw_arc(Vector2.ZERO, r, 0.0, TAU, 32,
				Color(C_ROOT.r, C_ROOT.g, C_ROOT.b, 0.58), 2.8, true)
	# Slow: dashed rotating frost segments
	elif st["slow"]:
		for i in 6:
			var a0 := float(i) * TAU / 6.0 + _t * 0.06
			draw_arc(Vector2.ZERO, r, a0, a0 + TAU / 12.5, 7,
					Color(C_SLOW.r, C_SLOW.g, C_SLOW.b, 0.52), 2.2, true)
	# Burn: pulsing orange ring (wider, outside body)
	if st["burn"]:
		var p := 0.35 + 0.18 * sin(_t * 0.38)
		draw_arc(Vector2.ZERO, r + 2.5, 0.0, TAU, 28,
				Color(C_BURN.r, C_BURN.g, C_BURN.b, p), 2.0, true)
	# Poison: subtle purple ring
	if st["poison"]:
		draw_arc(Vector2.ZERO, r + 1.5, 0.0, TAU, 24,
				Color(C_POISON.r, C_POISON.g, C_POISON.b, 0.38), 1.5, true)
	# Vulnerability: 4 crackling magenta arc segments (broken-armor look)
	if st["vuln"]:
		for i in 4:
			var a := float(i) * TAU / 4.0 + PI / 8.0 + _t * 0.04
			draw_arc(Vector2.ZERO, r + 3.8, a, a + TAU / 5.2, 9,
					Color(C_VULN.r, C_VULN.g, C_VULN.b, 0.44), 1.6, true)
...
func _draw_icons(st: Dictionary) -> void:
	...
	for i in n:
		...
		# Soft glow backing
		draw_circle(pos, ICON_R + 3.5, Color(col.r, col.g, col.b, 0.15))
		# Icon glyph
...
func _glyph_lock(c: Vector2, col: Color) -> void:
	...
	draw_arc(c + Vector2(0.0, -s * 0.22), s * 0.62, PI, 0.0, 10, col, 2.2, true)
	# Keyhole centre dot
	draw_circle(c + Vector2(0.0, s * 0.25), 1.4, Color(0.0, 0.0, 0.0, 0.72))
...
func _glyph_flame(c: Vector2, col: Color) -> void:
	...
	draw_colored_polygon(outer, Color(col.r, col.g, col.b, 0.88))
	# Inner hot-spot
	...
	draw_colored_polygon(inner, Color(1.0, 0.92, 0.35, 0.82))

func _glyph_skull(c: Vector2, col: Color) -> void:
	var s := ICON_R
	# Skull dome
	draw_circle(c, s, Color(col.r, col.g, col.b, 0.82))
	draw_circle(c, s, col, false, 1.2)
	# Eye sockets
	draw_circle(c + Vector2(-s * 0.36, -s * 0.10), s * 0.25, Color(0.0, 0.0, 0.0, 0.9))
	draw_circle(c + Vector2( s * 0.36, -s * 0.10), s * 0.25, Color(0.0, 0.0, 0.0, 0.9))
