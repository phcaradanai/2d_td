...
func _draw() -> void:
	if radius <= 0.0 or fade <= 0.01:
		return
	var pulse := 0.5 + sin(time * 0.7) * 0.5
	var base := _base_color()
	var line_alpha := minf((0.16 + pulse * 0.08) * fade, 0.24)
	if not active:
		line_alpha *= 0.45
	# [VISUAL-OPT] Removed large filled circle (was biggest cost).
	# Thin ring only, 24 segments instead of 96.
	draw_arc(Vector2.ZERO, radius * (0.995 + pulse * 0.006), 0.0, TAU, 24, Color(base.r, base.g, base.b, line_alpha), 1.5)
	_draw_role_ticks(base, pulse)
	if exact_radius_visible:
		draw_arc(Vector2.ZERO, radius, 0.0, TAU, 32, Color(0.82, 0.92, 1.0, 0.24 * fade), 1.25)
