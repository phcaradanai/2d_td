...
func _draw() -> void:
	...
	match icon_type:
		...
		_:
			draw_circle(Vector2.ZERO, 7, Color(color.r, color.g, color.b, _cap_alpha(0.35 * pulse)))
	if OS.is_debug_build() and label_text != "":
		draw_string(ThemeDB.fallback_font, Vector2(9, 4), label_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 10, Color(color.r, color.g, color.b, _cap_alpha(0.75)))

func _draw_reload_icon(pulse: float) -> void:
	draw_arc(Vector2.ZERO, 8.0, -PI * 0.25, PI * 1.35, 24, Color(color.r, color.g, color.b, _cap_alpha(0.62 * pulse)), 2.0)
	draw_line(Vector2(0, 0), Vector2(0, -6), Color(0.92, 0.96, 1.0, _cap_alpha(0.62 * pulse)), 1.5)
	draw_line(Vector2(0, 0), Vector2(5, 1), Color(0.92, 0.96, 1.0, _cap_alpha(0.62 * pulse)), 1.5)
...
func _draw_shield_icon(pulse: float) -> void:
	...
	draw_colored_polygon(pts, Color(color.r, color.g, color.b, _cap_alpha(0.16 * pulse)))
	draw_polyline(pts + PackedVector2Array([pts[0]]), Color(color.r, color.g, color.b, _cap_alpha(0.62 * pulse)), 1.5)

func _draw_cloak_icon(pulse: float) -> void:
	for i in range(4):
		var y := -7.0 + float(i) * 4.5
		draw_line(Vector2(-8 + sin(time * 2.4 + i) * 1.0, y), Vector2(8 + sin(time * 2.1 + i) * 1.0, y), Color(color.r, color.g, color.b, _cap_alpha(0.46 * pulse)), 1.25)

func _draw_air_icon(pulse: float) -> void:
	draw_arc(Vector2.ZERO, 9.0, 0, TAU, 24, Color(color.r, color.g, color.b, _cap_alpha(0.46 * pulse)), 1.25)
	draw_line(Vector2(-10, 0), Vector2(10, 0), Color(color.r, color.g, color.b, _cap_alpha(0.58 * pulse)), 1.5)
	draw_line(Vector2(0, -8), Vector2(0, 8), Color(color.r, color.g, color.b, _cap_alpha(0.30 * pulse)), 1.0)

# [VISUAL-OPT] Compact green cross/plus — healer identity marker.
func _draw_healer_id_icon(pulse: float) -> void:
	var s := 5.5
	var c := Color(color.r, color.g, color.b, _cap_alpha(0.68 * pulse))
	draw_line(Vector2(-s, 0.0), Vector2(s, 0.0), c, 2.5)
	draw_line(Vector2(0.0, -s), Vector2(0.0, s), c, 2.5)
	draw_circle(Vector2.ZERO, 1.5, Color(0.92, 1.0, 0.94, _cap_alpha(0.50 * pulse)))

func _draw_heal_cast_icon(pulse: float) -> void:
	var s := 7.0
	var c := Color(color.r, color.g, color.b, _cap_alpha(0.72 * pulse))
	draw_circle(Vector2.ZERO, 9.0, Color(color.r, color.g, color.b, _cap_alpha(0.10 * pulse)))
	draw_line(Vector2(-s, 0.0), Vector2(s, 0.0), c, 3.0)
	draw_line(Vector2(0.0, -s), Vector2(0.0, s), c, 3.0)
	draw_arc(Vector2.ZERO, 10.0, -PI * 0.15, PI * 1.15, 14, Color(color.r, color.g, color.b, _cap_alpha(0.30 * pulse)), 1.2)
...
func _draw_disruptor_id_icon(pulse: float) -> void:
	...
	draw_polyline(pts + PackedVector2Array([pts[0]]), Color(color.r, color.g, color.b, _cap_alpha(0.64 * pulse)), 1.8)
	draw_circle(Vector2.ZERO, 2.0, Color(color.r, color.g, color.b, _cap_alpha(0.52 * pulse)))
...
func _draw_bulwark_id_icon(pulse: float) -> void:
	...
	draw_polyline(pts + PackedVector2Array([pts[0]]), Color(color.r, color.g, color.b, _cap_alpha(0.64 * pulse)), 1.8)
	draw_circle(Vector2.ZERO, 2.2, Color(color.r, color.g, color.b, _cap_alpha(0.42 * pulse)))
