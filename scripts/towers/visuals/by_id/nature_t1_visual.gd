...
static func _draw_stroked_line(t: Node2D, from: Vector2, to: Vector2, color: Color, width: float, antialiased := true) -> void:
	TowerVisualDrawUtils.safe_draw_line(t, from, to, DETAIL_OUTLINE, width + 2.0, antialiased)
...
static func _draw_leaf_shape(t: Node2D, center: Vector2, radius: float, rot: float, fill: Color, line_color: Color) -> void:
	...
	for i in range(10):
		...
		pts.append(p)
	TowerVisualDrawUtils.safe_draw_polygon(t, _scaled(pts, center, 1.6), DETAIL_OUTLINE)
	TowerVisualDrawUtils.safe_draw_polygon(t, pts, fill)
...
static func _draw_nature_element_icon(t: Node2D, center: Vector2, radius: float, main_color: Color) -> void:
	...
	TowerVisualDrawUtils.safe_draw_polygon(t, _scaled(outer, center, 1.8), Color(token_color.r, token_color.g, token_color.b, 0.10))
	TowerVisualDrawUtils.safe_draw_polygon(t, outer, DETAIL_OUTLINE)
	TowerVisualDrawUtils.safe_draw_polygon(t, outer, Color(0.012, 0.030, 0.018, 0.86))
	TowerVisualDrawUtils.safe_draw_polygon(t, inner, Color(0.025, 0.095, 0.035, 0.76))
...
static func draw_contour(t: Node2D, detail_quality: int = TowerVisualDrawUtils.DetailQuality.MEDIUM) -> void:
	...
	if not _is_low_detail(detail_quality):
		# Leaf fins are decorative density; drop them at LOW.
...
static func draw_top(t: Node2D, main_color: Color, _secondary_color: Color, _core_color: Color, lvl: int, _size: float, _el_colors: Array[Color], detail_quality: int = TowerVisualDrawUtils.DetailQuality.MEDIUM) -> void:
	...
	if not _is_low_detail(detail_quality):
		...
		TowerVisualDrawUtils.safe_draw_circle(t, Vector2.ZERO, 18.5 + float(lvl) * 1.2, Color(main_color.r, main_color.g, main_color.b, 0.055))
	...
	TowerVisualDrawUtils.safe_draw_polygon(t, _scaled(body, Vector2.ZERO, -2.1), deep)
	...
	if not _is_low_detail(detail_quality):
		...
		TowerVisualDrawUtils.safe_draw_polygon(t, upper_plate, DETAIL_OUTLINE_SOFT)
		TowerVisualDrawUtils.safe_draw_polygon(t, _scaled(upper_plate, Vector2(3, -7), -1.1), Color(main_color.r, main_color.g, main_color.b, 0.24))
		_draw_stroked_polyline(t, upper_plate, Color(leaf.r, leaf.g, leaf.b, 0.45), 0.85)

		TowerVisualDrawUtils.safe_draw_polygon(t, lower_plate, DETAIL_OUTLINE_SOFT)
		TowerVisualDrawUtils.safe_draw_polygon(t, _scaled(lower_plate, Vector2(3, 7), -1.1), Color(main_color.r, main_color.g, main_color.b, 0.18))
	...
	TowerVisualDrawUtils.safe_draw_circle(t, Vector2(tip_x + 2.4, -5.6), 1.3, Color(0.75, 1.0, 0.48, 0.42))
	TowerVisualDrawUtils.safe_draw_circle(t, Vector2(tip_x + 2.4, 5.6), 1.3, Color(0.75, 1.0, 0.48, 0.36))
	...
	if not _is_low_detail(detail_quality):
		...
		for p in pulse_points:
			TowerVisualDrawUtils.safe_draw_circle(t, p, 4.7, Color(main_color.r, main_color.g, main_color.b, 0.055))
			TowerVisualDrawUtils.safe_draw_arc(t, p, 3.3, 0.0, TAU, 16, DETAIL_OUTLINE_SOFT, 1.0, true)
			TowerVisualDrawUtils.safe_draw_arc(t, p, 2.7, 0.0, TAU, 16, pulse_c, 0.8, true)
			TowerVisualDrawUtils.safe_draw_circle(t, p, 1.15, pulse_core)
