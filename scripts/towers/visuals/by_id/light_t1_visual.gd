...
static func _draw_light_element_icon(t: Node2D, center: Vector2, radius: float, main_color: Color) -> void:
	...
	TowerVisualDrawUtils.safe_draw_polygon(t, glow_poly, Color(token_color.r, token_color.g, token_color.b, 0.12))

	TowerVisualDrawUtils.safe_draw_polygon(t, outer_poly, DETAIL_OUTLINE)
	TowerVisualDrawUtils.safe_draw_polygon(t, outer_poly, Color(0.020, 0.024, 0.018, 0.84))
	TowerVisualDrawUtils.safe_draw_polygon(t, inner_poly, Color(0.055, 0.050, 0.018, 0.88))
	...
	TowerVisualDrawUtils.safe_draw_circle(t, center, radius * 0.92, Color(token_color.r, token_color.g, token_color.b, 0.08))
	...
	TowerVisualDrawUtils.safe_draw_polygon(t, star, DETAIL_OUTLINE)
	TowerVisualDrawUtils.safe_draw_polygon(t,
	...
	# Inner bright core.
	TowerVisualDrawUtils.safe_draw_circle(t, center, radius * 0.16, Color(1.0, 1.0, 0.82, 0.94))
	TowerVisualDrawUtils.safe_draw_circle(t, center, radius * 0.055, Color(1.0, 0.70, 0.12, 0.94))
...
static func draw_contour(t: Node2D, detail_quality: int = TowerVisualDrawUtils.DetailQuality.MEDIUM) -> void:
	...
	if not _is_low_detail(detail_quality):
		_outline_circle(t, Vector2(-10, 0), 4.2)
...
static func draw_top(t: Node2D, main_color: Color, _secondary_color: Color, _core_color: Color, lvl: int, _size: float, _el_colors: Array[Color], detail_quality: int = TowerVisualDrawUtils.DetailQuality.MEDIUM) -> void:
	...
	if detail_quality == TowerVisualDrawUtils.DetailQuality.LOW:
		TowerVisualDrawUtils.safe_draw_circle(t, Vector2.ZERO, 22.0 + float(lvl) * 1.5, Color(main_color.r, main_color.g, main_color.b, 0.055))
		TowerVisualDrawUtils.safe_draw_polygon(t, body, DETAIL_OUTLINE)
		TowerVisualDrawUtils.safe_draw_polygon(t, TowerVisualDrawUtils._expand_poly_from_center(body, -2.0), soft)
		_draw_light_element_icon(t, Vector2(-10, 0), 8.4 + float(lvl) * 0.15, main_color)
		TowerVisualDrawUtils.safe_draw_rect(t, Rect2(-2, -4, barrel_len, 8), deep)
		TowerVisualDrawUtils.safe_draw_line(t, Vector2(-2, 0), Vector2(barrel_len, 0), light, 2.2, true)
		var tip_x := barrel_len + 4.0
		TowerVisualDrawUtils.safe_draw_line(t, Vector2(tip_x - 5.0, -3.0), Vector2(tip_x, 0.0), light, 1.5, true)
		TowerVisualDrawUtils.safe_draw_line(t, Vector2(tip_x - 5.0, 3.0), Vector2(tip_x, 0.0), light, 1.5, true)
		TowerVisualDrawUtils.safe_draw_circle(t, Vector2(tip_x - 1.0, 0.0), 2.4, Color(main_color.r, main_color.g, main_color.b, 0.22))
		TowerVisualDrawUtils.safe_draw_circle(t, Vector2(tip_x - 1.0, 0.0), 1.2, Color(1.0, 0.98, 0.70, 0.80))
		TowerVisualDrawUtils.safe_draw_circle(t, Vector2(-10, 0), 1.35, Color(1.0, 1.0, 0.82, 0.92))
		return

	# Premium static halo. Cheap draw calls only; no particles/nodes.
	if not _is_low_detail(detail_quality):
		TowerVisualDrawUtils.safe_draw_circle(t, Vector2.ZERO, 22.0 + float(lvl) * 1.5, Color(main_color.r, main_color.g, main_color.b, 0.055))
		TowerVisualDrawUtils.safe_draw_arc(t, Vector2.ZERO, 18.5 + float(lvl) * 0.8, 0.0, TAU, 32, Color(main_color.r, main_color.g, main_color.b, 0.13), 1.0, true)
	...
	TowerVisualDrawUtils.safe_draw_polygon(t, upper_facet, DETAIL_OUTLINE_SOFT)
	TowerVisualDrawUtils.safe_draw_polygon(t, TowerVisualDrawUtils._expand_poly_from_center(upper_facet, -1.2), Color(light.r, light.g, light.b, 0.22))
	_draw_stroked_polyline(t, upper_facet, Color(light.r, light.g, light.b, 0.50), 0.9)

	TowerVisualDrawUtils.safe_draw_polygon(t, lower_facet, DETAIL_OUTLINE_SOFT)
	TowerVisualDrawUtils.safe_draw_polygon(t, TowerVisualDrawUtils._expand_poly_from_center(lower_facet, -1.2), Color(light.r, light.g, light.b, 0.14))
	...
	for p in halo_points:
		if not _is_low_detail(detail_quality):
			TowerVisualDrawUtils.safe_draw_circle(t, p, 5.5, Color(main_color.r, main_color.g, main_color.b, 0.08))
			TowerVisualDrawUtils.safe_draw_arc(t, p, 4.2, 0.0, TAU, 18, DETAIL_OUTLINE_SOFT, 1.2, true)
			TowerVisualDrawUtils.safe_draw_arc(t, p, 3.4, 0.0, TAU, 18, halo_c, 1.0, true)
		TowerVisualDrawUtils.safe_draw_circle(t, p, 1.6, halo_core)
