...
const VERIFIER_SCRIPT := preload("res://scripts/debug/tower_ability_verifier.gd")
...
func _run() -> void:
	...
	root_scene.name = "TowerAbilityVerificationTestScene"
	...
	if failed > 0:
		push_error("Tower ability verification failed: %d checks failed." % failed)
		quit(1)
	else:
		print("[TEST][PASS] Tower ability verification checks passed.")
		quit(0)
